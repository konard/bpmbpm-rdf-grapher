# Semantic ARIS — Подробное введение

## 1. Обзор проекта RDF Grapher ver9d

### 1.1 Что такое RDF Grapher

RDF Grapher — это веб-приложение для парсинга RDF данных и их визуализации в виде графов. Проект эволюционировал от простого RDF Viewer (ver1-ver3) до полноценной системы моделирования бизнес-процессов на основе семантических технологий (ver4-ver9).
...
| **N3.Store** | In-memory quadstore для хранения RDF данных |

Приложение работает полностью на стороне клиента (в браузере) и не требует серверной части.
...
| Граф | URI | Назначение |
|------|-----|------------|
| Дерево процессов | `vad:ptree` | Хранение концептов процессов (TypeProcess) |
| Дерево исполнителей | `vad:rtree` | Хранение концептов исполнителей (TypeExecutor) |
| Технологическое приложение | `vad:techtree` | Метаданные: методы, предикаты, типы |
| Схема процесса | `vad:t_*` | VAD-диаграмма процесса (VADProcessDia) |
| Виртуальные данные | `vad:vt_*` | Автоматически вычисляемые данные |
...
### 4.3 Пример: SPARQL-запрос для построения RACI-матрицы
...
# Получение связей процесс-исполнитель для RACI
