...
## auto
что создается автоматом:
- имя (id, label) TriG по имени Процесса
```
...
techRDF:
Если у концепта есть TriG, то индивид Detailed.
Если схема индивида (подпроцесс) является дочерней (hasParent) к текущей схеме, то  vad:processSubtype vad:DetailedChild ;  
иначе - чужой.  
от этого опредеяется раскраска. 

## think about it
- иерархические схемы
- индивид одноименный в схеме (повтор)
