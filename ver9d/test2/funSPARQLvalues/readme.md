...
---
## 2 Поля `uri` и `label` 
Поля `uri` и `label` в результатах, возвращаемых функциями `funSPARQLvalues` и `funSPARQLvaluesComunica`, добавляются **искусственно** для обратной совместимости и удобства отображения в пользовательском интерфейсе. Рассмотрим подробно, откуда они берутся и как выглядит итоговый массив.

---

## 1. Происхождение `uri` и `label`

### В `funSPARQLvalues`
- **`uri`** – берётся из значения переменной, указанной во втором аргументе (`variableName`). Если этот аргумент не передан, используется первая переменная из списка `SELECT`.  
  Код:
  ```javascript
  ...
  uri: value
  ```
- **`label`** – если в результатах запроса присутствует переменная `?label` (т.е. `binding['label']` существует), то её значение используется. В противном случае вызывается функция `getPrefixedName(value, currentPrefixes)`, которая преобразует полный URI в сокращённую форму с префиксом (например, `ex:Person/Alice`).  
  Код:
  ```javascript
  const label = binding['label'] || getPrefixedName(value, currentPrefixes);
  ```

### В `funSPARQLvaluesComunica`
- Сначала формируется объект `resultObj`, содержащий **все переменные**, возвращённые SPARQL-запросом (ключи – имена переменных, значения – строки с URI/литералами).
- Затем для обратной совместимости добавляются поля `uri` и `label`:
  - **`uri`** – дублирует значение переменной, имя которой передано как `variableName` (или используется значение по умолчанию `'value'`).
  - **`label`** – если в объекте уже есть ключ `'label'` (т.е. запрос содержал переменную `?label`), то он остаётся. Иначе `label` вычисляется через `getPrefixedName` для значения `uri`.
  Код:
  ```javascript
...
**Важно:** Функция `getPrefixedName` не определена в приведённом файле, но, судя по контексту, она должна использовать глобальный объект `currentPrefixes` для преобразования полного URI в сокращённую форму (например, `http://example.org/Person/Alice` → `ex:Alice`).

---

## 2. Как выглядит итоговый массив?

### Для `funSPARQLvalues` (простая реализация)
Массив состоит из объектов, содержащих **только** `uri` и `label` (даже если в запросе было несколько переменных).  
Пример:
```javascript
[
  { uri: "http://example.org/Person/Alice", label: "Alice" },
  { uri: "http://example.org/Person/Bob",   label: "Bob" }
]
```

### Для `funSPARQLvaluesComunica` (полноценный SPARQL)
Каждый объект включает **все переменные запроса** плюс служебные `uri` и `label`.  
Пример для запроса:
```sparql
SELECT ?person ?name ?age WHERE { ... }
```
при вызове `funSPARQLvaluesComunica(query, 'person')` результат будет:
```javascript
[
  {
  ...
  },
  {
    ...
    name: "Bob",
    // age отсутствует, так как в данных его нет
...
Если в запросе была переменная `?label`, она будет присутствовать как отдельное поле, а `label` (искусственное) продублирует её значение (если не переопределено через `getPrefixedName`).  
Пример с `?label`:
```javascript
[
  {
    person: "http://example.org/Person/Alice",
    label: "Alice",          // переменная ?label
    uri: "http://example.org/Person/Alice",
    label_dup: "Alice"       // искусственное поле label (совпадает)
  }
]
```
...
## 3. Зачем это нужно?
- **`uri`** – унифицирует доступ к основному идентификатору сущности, чтобы в интерфейсе можно было всегда обращаться к `item.uri`, не задумываясь, как называлась переменная в конкретном запросе.
- **`label`** – обеспечивает наличие человекочитаемого имени для отображения (либо из данных, либо сгенерированного из URI).

Таким образом, разработчик может работать с результатами единообразно, независимо от сложности запроса.

## 3 Как формируется label
Отсутствие в исходных данных троек с предикатом `label` (например, `rdfs:label`) **не нарушает работу** функций `funSPARQLvalues` и `funSPARQLvaluesComunica`. Обе функции гарантированно возвращают объекты с полями `uri` и `label` для каждого результата, даже если в запросе не было переменной `?label` или она осталась несвязанной из-за отсутствия данных.

### Как формируется `label` в зависимости от наличия данных

#### 1. Если в запросе **присутствует** переменная `?label` и в данных есть соответствующие тройки
- `funSPARQLvalues`: значение переменной `label` (из `binding`) попадает в поле `label` результата.
- `funSPARQLvaluesComunica`: в объекте `resultObj` появляется ключ `label` с этим значением, и он же остаётся в финальном поле `label`.

#### 2. Если в запросе **нет** переменной `?label` или она **не получила значения** (например, нет троек с предикатом `rdfs:label`)
- Обе функции вызывают вспомогательную функцию `getPrefixedName(uri, currentPrefixes)`, которая преобразует полный URI (значение основной переменной) в сокращённую форму с префиксом (например, `http://example.org/Person/Alice` → `ex:Alice`). Это значение и становится полем `label`.

Таким образом, поле `label` **всегда будет присутствовать** в каждом объекте результата, независимо от содержимого хранилища.

### Влияет ли отсутствие `label` на другие аспекты?
- **Нет**, функции не требуют обязательного наличия троек с `label`. Они корректно обрабатывают любые SPARQL-запросы, возвращая все затребованные переменные (в случае `funSPARQLvaluesComunica`) и дополнительно добавляя `uri`/`label`.
- Единственное различие: если в запросе была переменная `?label` и данные её не содержат, то в объекте `resultObj` (для `funSPARQLvaluesComunica`) не будет ключа `label` среди оригинальных переменных, но искусственное поле `label` всё равно будет добавлено на основе URI. Для пользователя это означает, что отображаемое имя будет сгенерировано из URI, а не взято из данных.

### Пример
Исходные данные (без троек `rdfs:label`):
```turtle
...
```
Результат `funSPARQLvaluesComunica`:
```json
[
  { "person": "http://example.org/Person/Alice", "uri": "http://example.org/Person/Alice", "label": "ex:Alice" },
  { "person": "http://example.org/Person/Bob",   "uri": "http://example.org/Person/Bob",   "label": "ex:Bob" }
]
```
Поле `label` сгенерировано из URI с помощью `getPrefixedName`.

### Вывод
Функции разработаны с учётом возможного отсутствия явных меток в данных и всегда предоставляют замену в виде сокращённого URI. Поэтому отсутствие предиката `label` в хранилище не приводит к ошибкам или пустым значениям – работа остаётся стабильной.

---
## funConceptList_v2 
Начиная с https://github.com/bpmbpm/rdf-grapher/issues/425 замена на funConceptList_v2 
