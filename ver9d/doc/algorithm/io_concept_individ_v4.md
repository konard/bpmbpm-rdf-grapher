...
<!-- Обновление: https://github.com/bpmbpm/rdf-grapher/issues/313 (два режима hasNext) -->
<!-- Обновление: https://github.com/bpmbpm/rdf-grapher/issues/319 (test_individ.html) -->
<!-- Pull Request: https://github.com/bpmbpm/rdf-grapher/pull/310, #312, #314, #320 -->
<!-- Дата: 2026-02-08 -->
...
13. **Два режима vad:hasNext** (issue #313, PR #314) — при создании индивида процесса: «на существующий» (индивиды в TriG) и «на любой» (все концепты из ptree)
...
4. Выбирает **режим vad:hasNext** (issue #313, PR #314):
   - **«vad:hasNext на существующий»** (по умолчанию) — справочник содержит только **индивиды процесса**, уже добавленные в выбранный TriG
   - **«vad:hasNext на любой»** — справочник содержит **все концепты процессов** из ptree (предыдущее поведение)
5. (Опционально) Из справочника выбирает один или несколько объектов для `vad:hasNext` — множественный выбор через чекбоксы
6. Система автоматически формирует `ExecutorGroup` с ID вида `ExecutorGroup_{processId}`
7. Формируется SPARQL INSERT DATA и отображается в панели «Result in SPARQL»

> **Примечание (issue #313):** Режим «на существующий» позволяет создавать связи `vad:hasNext` только на те процессы, которые уже являются индивидами в текущей схеме. Это обеспечивает целостность схемы и предотвращает ссылки на несуществующие индивиды.
...
#### Запрос 3: Получение элементов для справочника hasNext

> **issue #313 (PR #314):** Справочник hasNext поддерживает два режима, каждый со своим SPARQL-запросом.

**Режим «vad:hasNext на существующий» (по умолчанию):**

```sparql
# Функция: getIndividsInTrig(trigUri)
# Выполняется: manual quadstore search
# Примечание: возвращает индивиды процесса, уже добавленные в выбранный TriG
...
# Результат: только индивиды данного TriG (например, 2 элемента для t_p1)
```

**Режим «vad:hasNext на любой»:**
...
# Результат: все концепты процессов из ptree (например, 11 элементов)
```
...
*Документ создан в рамках PR #310 по issue #309, обновлён в PR #312 по issue #311, обновлён в PR #314 по issue #313 (два режима hasNext), обновлён в PR #320 по issue #319*
