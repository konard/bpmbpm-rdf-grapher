...
Документ дополняет [store_concept_v3.md](store_concept_v3.md) и фиксирует **текущее состояние** quadstore в ver9d после успешной миграции (issue #324, #325). Документ также анализирует, какие массивы ещё могут быть удалены и заменены на прямые SPARQL-запросы к единственному источнику данных — `currentStore` (N3.Store).
...
Анализ файла `9_vadlib/vadlib.js` и других модулей ver9d выявил следующие массивы и кэши, которые потенциально могут быть заменены SPARQL-запросами:
