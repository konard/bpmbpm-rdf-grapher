...
**Однако** на практике N3.Writer (конкретная версия, используемая в проекте) может НЕ выводить кирилличные ID как prefixed name, а записывать их как полные URI:

```turtle
<http://example.org/vad#яяя22> a vad:TypeProcess;
    rdfs:label "яяя6666";
    vad:hasParentObj vad:p1
```

### Решение (issue #435): postprocessing через replaceFullUrisWithPrefixes

Для корректного отображения кирилличных ID применяется постобработка через функцию `replaceFullUrisWithPrefixes` (определена в `3_sd_logic.js`).

Функция заменяет полные URI `<http://example.org/vad#яяя22>` на сокращённую форму `vad:яяя22` с помощью регулярного выражения. Это корректное поведение: данные в quadstore хранятся как полные URI (что правильно с точки зрения RDF), а для отображения применяется постобработка.

В версии issue #435 эта постобработка применяется при:
1. Сериализации через `serializeStoreToTriG` в `3_sd_logic.js` (уже реализовано)
2. Обновлении отображения quadstore `updateQuadstoreDisplay` в `2_triplestore_ui.js` (исправлено в issue #435)
3. Сохранении файла `saveAsFile` в `2_triplestore_ui.js` (исправлено в issue #435)
...
После применения `replaceFullUrisWithPrefixes` в квадсторе будет отображаться:
...
3. **Кириллические буквы допустимы** — `яяя`, `процесс1`, `ПроцессА`
4. **Prefix должен быть объявлен** в N3.Writer
5. **replaceFullUrisWithPrefixes должна вызываться** после N3.Writer.end()

### Метод именования с кириллицей
...
// может выводиться: <http://example.org/vad#яяя> (полный URI)
// После replaceFullUrisWithPrefixes: vad:яяя (сокращённый URI)
```
...
| Кирилличный ID (корректный) | `яяя` | `vad:яяя` ✓ (после postprocessing) | N3.Writer может выводить полный URI, replaceFullUrisWithPrefixes исправляет это |
