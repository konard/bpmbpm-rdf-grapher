...
function getObjectsForParentSelector(typeUri, graphUri) {
    ...
    if (typeof funSPARQLvalues === 'function') {
        return funSPARQLvalues(sparqlQuery, 'object');
    }

    return getObjectsByTypeManual(typeUri, graphUri);
}
...
function displayIntermediateSparql() {
    ...
    let sparqlText = '# ===== Промежуточные SPARQL запросы =====\n\n';
...
function createNewConceptSparql() {
    ...
    const id = document.getElementById('new-concept-id')?.value?.trim();
    if (!id) {
