import { translateNamed, translateTerm, translateTripleCollection, translateTripleNesting, } from '@traqula/algebra-transformations-1-1';
import { findPatternBoundedVars, } from '@traqula/rules-sparql-1-2';
import { termToString } from 'rdf-string';
const reificationIri = 'http://www.w3.org/1999/02/22-rdf-syntax-ns#reifies';
export const translateTerm12 = {
    name: 'translateTerm',
    fun: $ => (C, term) => {
        if (C.astFactory.isTermTriple(term)) {
            return C.dataFactory.quad($.SUBRULE(translateTerm12, term.subject), $.SUBRULE(translateTerm12, term.predicate), $.SUBRULE(translateTerm12, term.object));
        }
...
export const translateTripleCollection12 = {
    ...
    fun: s => (c, collection, result) => {
        if (c.astFactory.isTripleCollectionReifiedTriple(collection)) {
            ...
            translateTripleCollection.fun(s)(c, collection, result);
        }
    },
};
/**
...
export const translateTripleNesting12 = {
    ...
    fun: s => (c, triple, result) => {
        translateTripleNesting.fun(s)(c, triple, result);
