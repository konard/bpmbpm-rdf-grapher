...
function clearOrDrop(operation) {
    return {
        ...
        impl: ({ ACTION, SUBRULE1, CONSUME, OPTION }) => (C) => {
            ...
            return ACTION(() => C.astFactory.updateOperationClearDrop(unCapitalize(operation.name), Boolean(silent), destination, C.astFactory.sourceLocation(opToken, destination)));
        },
        gImpl: ({ SUBRULE, PRINT_WORD, PRINT_ON_EMPTY }) => (ast, { astFactory: F }) => {
            F.printFilter(ast, () => {
...
function copyMoveAddOperation(operation) {
    return {
        ...
        impl: ({ ACTION, CONSUME, SUBRULE1, SUBRULE2, OPTION }) => (C) => {
            ...
            return ACTION(() => C.astFactory.updateOperationAddMoveCopy(unCapitalize(operation.name), source, destination, Boolean(silent), C.astFactory.sourceLocation(op, destination)));
        },
        gImpl: ({ SUBRULE, PRINT_WORD, PRINT_ON_EMPTY }) => (ast, { astFactory: F }) => {
            F.printFilter(ast, () => {
                PRINT_ON_EMPTY(operation.name.toUpperCase(), ' ');
