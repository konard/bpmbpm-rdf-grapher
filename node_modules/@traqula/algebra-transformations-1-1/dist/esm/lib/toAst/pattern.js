...
import { translateAlgDatasetClauses, translateAlgDistinct, translateAlgExtend, translateAlgOrderBy, translateAlgPattern, translateAlgReduced, translateAlgTerm, } from './general.js';
import { translateAlgPathComponent } from './path.js';
import { translateAlgConstruct, translateAlgProject } from './queryUnit.js';
export const translateAlgPatternIntoGroup = {
...
export const translateAlgSinglePattern = {
...
export const translateAlgPatternNew = {
...
export const translateAlgPath = {
    name: 'translatePath',
    fun: ({ SUBRULE }) => ({ astFactory: F }, op) => F.patternBgp([
        F.triple(SUBRULE(translateAlgTerm, op.subject), SUBRULE(translateAlgPathComponent, op.predicate), SUBRULE(translateAlgTerm, op.object)),
    ], F.gen()),
};
/**
 ...
 */
export const translateAlgFilter = {
...
export const translateAlgGraph = {
    name: 'translateGraph',
    fun: ({ SUBRULE }) => ({ astFactory: F }, op) => F.patternGraph(SUBRULE(translateAlgTerm, op.name), [SUBRULE(translateAlgPatternNew, op.input)].flat(), F.gen()),
};
/**
 * A group needs to be handled by {@link translateAlgProject}
 */
export const translateAlgGroup = {
    name: 'translateGroup',
...
export const translateAlgLeftJoin = {
    name: 'translateLeftJoin',
    fun: ({ SUBRULE }) => ({ astFactory: F }, op) => {
        const leftJoin = F.patternOptional(SUBRULE(operationAlgInputAsPatternList, op.input[1]), F.gen());
        if (op.expression) {
            leftJoin.patterns.push(F.patternFilter(SUBRULE(translateAlgPureExpression, op.expression), F.gen()));
        }
...
export const translateAlgMinus = {
    name: 'translateMinus',
    fun: ({ SUBRULE }) => ({ astFactory: F }, op) => [
        SUBRULE(translateAlgPatternNew, op.input[0]),
...
export const translateAlgService = {
    name: 'translateService',
    fun: ({ SUBRULE }) => ({ astFactory: F }, op) => F.patternService(SUBRULE(translateAlgTerm, op.name), SUBRULE(operationAlgInputAsPatternList, op.input), op.silent, F.gen()),
};
/**
 * Unwrap single group patterns, create array if it was not yet.
 */
export const operationAlgInputAsPatternList = {
 ...
 * Similar to {@link translateAlgDistinct}
 */
export const translateAlgSlice = {
    name: 'translateSlice',
    fun: ({ SUBRULE }) => ({ astFactory: F }, op) => {
        const result = SUBRULE(translateAlgPatternIntoGroup, op.input);
        const query = result.patterns[0];
...
export const algWrapInPatternGroup = {
...
export const translateAlgUnion = {
    name: 'translateUnion',
    fun: ({ SUBRULE }) => ({ astFactory: F }, op) => F.patternUnion(op.input.map(operation => SUBRULE(algWrapInPatternGroup, SUBRULE(translateAlgPatternNew, operation))), F.gen()),
};
export const translateAlgValues = {
    ...
    fun: ({ SUBRULE }) => ({ astFactory: F }, op) => F.patternValues(op.variables.map(variable => F.termVariable(variable.value, F.gen())), op.bindings.map((binding) => {
        ...
        for (const v of op.variables) {
            const s = v.value;
            if (binding[s]) {
                result[s] = SUBRULE(translateAlgTerm, binding[s]);
            }
            else {
                result[s] = undefined;
            }
