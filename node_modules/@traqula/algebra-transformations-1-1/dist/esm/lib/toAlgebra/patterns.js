...
export const translateExpression = {
    ...
    fun: ({ SUBRULE }) => ({ astFactory: F, algebraFactory: AF }, expr) => {
        ...
        if (F.isExpressionAggregate(expr)) {
            return AF.createAggregateExpression(expr.aggregation, SUBRULE(translateExpression, expr.expression[0]), expr.distinct, F.isExpressionAggregateSeparator(expr) ? expr.separator : undefined);
        }
        if (F.isExpressionFunctionCall(expr)) {
            // Outdated typings
            return AF.createNamedExpression(SUBRULE(translateNamed, expr.function), expr.args.map(subExpr => SUBRULE(translateExpression, subExpr)));
        }
        if (F.isExpressionOperator(expr)) {
            return AF.createOperatorExpression(expr.operator, expr.args.map(subExpr => SUBRULE(translateExpression, subExpr)));
        }
        if (F.isExpressionPatternOperation(expr)) {
            return AF.createExistenceExpression(expr.operator === 'notexists', SUBRULE(translateGraphPattern, expr.args));
        }
        throw new Error(`Unknown expression: ${JSON.stringify(expr)}`);
    },
};
export const translateGraphPattern = {
    ...
    fun: ({ SUBRULE }) => ({ astFactory: F, algebraFactory: AF, useQuads }, pattern) => {
        ...
        if (F.isPatternUnion(pattern)) {
            return AF.createUnion(pattern.patterns.map((group) => SUBRULE(translateGraphPattern, group)));
        }
        ...
        if (F.isPatternGraph(pattern)) {
            ...
            if (useQuads) {
                ...
                result = AF.createGraph(result, SUBRULE(translateTerm, pattern.name));
            }
            return result;
        }
        ...
        if (F.isPatternGroup(pattern)) {
            ...
            const expressions = filters.map(filter => SUBRULE(translateExpression, filter.expression));
 ...
 */
export const translateBgp = {
    name: 'translateBgp',
    fun: ({ SUBRULE }) => ({ astFactory: F, algebraFactory: AF }, bgp) => {
        let patterns = [];
        const joins = [];
        const flattenedTriples = [];
...
export const accumulateGroupGraphPattern = {
    ...
    fun: ({ SUBRULE }) => ({ astFactory: F, algebraFactory: AF }, algebraOp, pattern) => {
        ...
        if (F.isPatternBind(pattern)) {
            return AF.createExtend(algebraOp, SUBRULE(translateTerm, pattern.variable), SUBRULE(translateExpression, pattern.expression));
        }
        if (F.isPatternService(pattern)) {
            // Transform to group so child-nodes get parsed correctly
            const group = F.patternGroup(pattern.patterns, pattern.loc);
            const A = AF.createService(SUBRULE(translateGraphPattern, group), SUBRULE(translateTerm, pattern.name), pattern.silent);
