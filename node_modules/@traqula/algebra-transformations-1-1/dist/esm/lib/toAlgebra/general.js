...
export const registerContextDefinitions = {
...
export const translateInlineData = {
    name: 'translateInlineData',
    fun: ({ SUBRULE }) => ({ algebraFactory: AF }, values) => {
        const variables = values.variables.map(x => SUBRULE(translateTerm, x));
        const bindings = values.values.map((binding) => {
            const map = {};
            for (const [key, value] of Object.entries(binding)) {
                if (value !== undefined) {
                    map[key] = SUBRULE(translateTerm, value);
                }
            }
            return map;
        });
    ...
    fun: ({ SUBRULE }) => ({ algebraFactory: AF, variables }, res) => {
        ...
        return util.mapOperation(res, {
            ...
            [Algebra.Types.CONSTRUCT]: {
                ...
                transform: constructOp => AF.createConstruct(SUBRULE(translateBlankNodesToVariables, constructOp.input), constructOp.template),
            },
            [Algebra.Types.DELETE_INSERT]: {
                preVisitor: () => ({ continue: false }),
                transform: delInsOp => 
                // Make sure blank nodes remain in the INSERT block, but do update the WHERE block
                AF.createDeleteInsert(delInsOp.delete, delInsOp.insert, delInsOp.where && SUBRULE(translateBlankNodesToVariables, delInsOp.where)),
            },
        });
    },
};
