...
exports.removeAlgQuadsRecursive = {
    ...
    fun: ({ SUBRULE }) => ({ algebraFactory: AF }, unknownVal, graphs) => {
        ...
        const operationGraphNames = {};
        for (const [key, value] of Object.entries(knownOp)) {
            const newGraphs = [];
            result[key] = SUBRULE(exports.removeAlgQuadsRecursive, value, newGraphs);
        ...
        if (graphNameSet.length > 0) {
            ...
            if (graphNameSet.length === 1 && ![index_js_1.types.PROJECT, index_js_1.types.SERVICE].includes(knownOp.type)) {
                graphs.push(operationGraphNames[graphNameSet[0]]);
            }
            else if (knownOp.type === index_js_1.types.BGP) {
                // This is the specific case that `op` got changed because of using quads. -
                return SUBRULE(exports.splitAlgBgpToGraphs, knownOp, keyGraphs.patterns);
            }
            else {
