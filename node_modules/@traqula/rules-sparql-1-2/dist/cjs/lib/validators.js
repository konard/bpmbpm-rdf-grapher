...
exports.langTagHasCorrectRange = langTagHasCorrectRange;
exports.findPatternBoundedVars = findPatternBoundedVars;
const AstFactory_js_1 = require("./AstFactory.js");
const F = new AstFactory_js_1.AstFactory();
...
function findPatternBoundedVars(iter, boundedVars) {
    if (F.isQuery(iter) || F.isUpdate(iter)) {
        ...
        for (const annotation of iter.annotations ?? []) {
            findPatternBoundedVars(F.isTripleCollection(annotation) ? annotation : annotation.val, boundedVars);
        }
    }
