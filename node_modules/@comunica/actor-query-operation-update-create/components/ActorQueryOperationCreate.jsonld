{
  "@context": [
    "https://linkedsoftwaredependencies.org/bundles/npm/@comunica/actor-query-operation-update-create/^5.0.0/components/context.jsonld",
    "https://linkedsoftwaredependencies.org/bundles/npm/@comunica/core/^5.0.0/components/context.jsonld",
    "https://linkedsoftwaredependencies.org/bundles/npm/@comunica/bus-query-operation/^5.0.0/components/context.jsonld"
  ],
  "@id": "npmd:@comunica/actor-query-operation-update-create",
  "components": [
    {
      "@id": "caqouc:components/ActorQueryOperationCreate.jsonld#ActorQueryOperationCreate",
      "@type": "Class",
      "requireElement": "ActorQueryOperationCreate",
      "extends": [
        {
          "@type": "GenericComponentExtension",
          "component": "cbqo:components/ActorQueryOperationTypedMediated.jsonld#ActorQueryOperationTypedMediated",
          "genericTypeInstances": [
            {
              "@type": "ParameterRangeIntersection",
              "parameterRangeElements": [
                {
                  "@type": "ParameterRangeWildcard"
                },
                {
                  "@type": "ParameterRangeWildcard"
                }
              ]
            }
          ]
        }
      ],
      "comment": "A [Query Operation](https://github.com/comunica/comunica/tree/master/packages/bus-query-operation) actor that handles SPARQL create operations.",
      "parameters": [
        {
          "@id": "caqouc:components/ActorQueryOperationCreate.jsonld#ActorQueryOperationCreate_args_mediatorUpdateQuads",
          "range": "cc:components/Mediator.jsonld#Mediator",
          "comment": "The RDF Update Quads mediator"
        },
        {
          "@id": "caqouc:components/ActorQueryOperationCreate.jsonld#ActorQueryOperationCreate_args_mediatorQueryOperation",
          "range": "cc:components/Mediator.jsonld#Mediator"
        },
        {
          "@id": "caqouc:components/ActorQueryOperationCreate.jsonld#ActorQueryOperationCreate_args_name",
          "range": "xsd:string",
          "default": {
            "@id": "rdf:subject"
          },
          "comment": "The name for this actor."
        },
        {
          "@id": "caqouc:components/ActorQueryOperationCreate.jsonld#ActorQueryOperationCreate_args_bus",
          "range": {
            "@type": "ParameterRangeGenericComponent",
            "component": "cc:components/Bus.jsonld#Bus",
            "genericTypeInstances": [
              {
                "@type": "ParameterRangeGenericComponent",
                "component": "cc:components/Actor.jsonld#Actor",
                "genericTypeInstances": [
                  {
                    "@type": "ParameterRangeGenericTypeReference",
                    "parameterRangeGenericType": "npmd:@comunica/actor-abstract-mediatyped/^5.0.0/components/ActorAbstractMediaTyped.jsonld#ActorAbstractMediaTyped__generic_I"
                  },
                  {
                    "@type": "ParameterRangeGenericTypeReference",
                    "parameterRangeGenericType": "npmd:@comunica/actor-abstract-mediatyped/^5.0.0/components/ActorAbstractMediaTyped.jsonld#ActorAbstractMediaTyped__generic_T"
                  },
                  {
                    "@type": "ParameterRangeGenericTypeReference",
                    "parameterRangeGenericType": "npmd:@comunica/actor-abstract-mediatyped/^5.0.0/components/ActorAbstractMediaTyped.jsonld#ActorAbstractMediaTyped__generic_O"
                  },
                  {
                    "@type": "ParameterRangeGenericTypeReference",
                    "parameterRangeGenericType": "npmd:@comunica/actor-abstract-mediatyped/^5.0.0/components/ActorAbstractMediaTyped.jsonld#ActorAbstractMediaTyped__generic_TS"
                  }
                ]
              },
              {
                "@type": "ParameterRangeGenericTypeReference",
                "parameterRangeGenericType": "npmd:@comunica/actor-abstract-mediatyped/^5.0.0/components/ActorAbstractMediaTyped.jsonld#ActorAbstractMediaTyped__generic_I"
              },
              {
                "@type": "ParameterRangeGenericTypeReference",
                "parameterRangeGenericType": "npmd:@comunica/actor-abstract-mediatyped/^5.0.0/components/ActorAbstractMediaTyped.jsonld#ActorAbstractMediaTyped__generic_T"
              },
              {
                "@type": "ParameterRangeGenericTypeReference",
                "parameterRangeGenericType": "npmd:@comunica/actor-abstract-mediatyped/^5.0.0/components/ActorAbstractMediaTyped.jsonld#ActorAbstractMediaTyped__generic_O"
              },
              {
                "@type": "ParameterRangeGenericTypeReference",
                "parameterRangeGenericType": "npmd:@comunica/actor-abstract-mediatyped/^5.0.0/components/ActorAbstractMediaTyped.jsonld#ActorAbstractMediaTyped__generic_TS"
              }
            ]
          },
          "default": {
            "@id": "cbqo:components/ActorQueryOperation.jsonld#ActorQueryOperation_default_bus",
            "@type": "cbqo:components/BusQueryOperation.jsonld#BusQueryOperation"
          },
          "comment": "The bus this actor subscribes to."
        },
        {
          "@id": "caqouc:components/ActorQueryOperationCreate.jsonld#ActorQueryOperationCreate_args_busFailMessage",
          "range": {
            "@type": "ParameterRangeUnion",
            "parameterRangeElements": [
              "xsd:string",
              {
                "@type": "ParameterRangeUndefined"
              }
            ]
          },
          "default": "Query operation processing failed: none of the configured actors were able to handle the operation type ${action.operation.type}",
          "comment": "The message that will be configured in the bus for reporting failures.  This message may be a template string that contains references to the executed `action`. For example, the following templated string is allowed: \"RDF dereferencing failed: no actors could handle ${action.handle.mediaType}\""
        },
        {
          "@id": "caqouc:components/ActorQueryOperationCreate.jsonld#ActorQueryOperationCreate_args_beforeActors",
          "range": {
            "@type": "ParameterRangeUnion",
            "parameterRangeElements": [
              {
                "@type": "ParameterRangeArray",
                "parameterRangeValue": {
                  "@type": "ParameterRangeGenericComponent",
                  "component": "cc:components/Actor.jsonld#Actor",
                  "genericTypeInstances": [
                    {
                      "@type": "ParameterRangeGenericTypeReference",
                      "parameterRangeGenericType": "npmd:@comunica/actor-abstract-mediatyped/^5.0.0/components/ActorAbstractMediaTyped.jsonld#ActorAbstractMediaTyped__generic_I"
                    },
                    {
                      "@type": "ParameterRangeGenericTypeReference",
                      "parameterRangeGenericType": "npmd:@comunica/actor-abstract-mediatyped/^5.0.0/components/ActorAbstractMediaTyped.jsonld#ActorAbstractMediaTyped__generic_T"
                    },
                    {
                      "@type": "ParameterRangeGenericTypeReference",
                      "parameterRangeGenericType": "npmd:@comunica/actor-abstract-mediatyped/^5.0.0/components/ActorAbstractMediaTyped.jsonld#ActorAbstractMediaTyped__generic_O"
                    },
                    {
                      "@type": "ParameterRangeGenericTypeReference",
                      "parameterRangeGenericType": "npmd:@comunica/actor-abstract-mediatyped/^5.0.0/components/ActorAbstractMediaTyped.jsonld#ActorAbstractMediaTyped__generic_TS"
                    }
                  ]
                }
              },
              {
                "@type": "ParameterRangeUndefined"
              }
            ]
          },
          "comment": "Actor that must be registered in the bus before this actor."
        }
      ],
      "memberFields": [
        {
          "@id": "caqouc:components/ActorQueryOperationCreate.jsonld#ActorQueryOperationCreate__member_mediatorUpdateQuads",
          "memberFieldName": "mediatorUpdateQuads",
          "range": "cc:components/Mediator.jsonld#Mediator"
        },
        {
          "@id": "caqouc:components/ActorQueryOperationCreate.jsonld#ActorQueryOperationCreate__member_constructor",
          "memberFieldName": "constructor"
        },
        {
          "@id": "caqouc:components/ActorQueryOperationCreate.jsonld#ActorQueryOperationCreate__member_testOperation",
          "memberFieldName": "testOperation"
        },
        {
          "@id": "caqouc:components/ActorQueryOperationCreate.jsonld#ActorQueryOperationCreate__member_runOperation",
          "memberFieldName": "runOperation"
        }
      ],
      "constructorArguments": [
        {
          "@id": "caqouc:components/ActorQueryOperationCreate.jsonld#ActorQueryOperationCreate_args__constructorArgument",
          "fields": [
            {
              "keyRaw": "mediatorUpdateQuads",
              "value": {
                "@id": "caqouc:components/ActorQueryOperationCreate.jsonld#ActorQueryOperationCreate_args_mediatorUpdateQuads"
              }
            },
            {
              "keyRaw": "mediatorQueryOperation",
              "value": {
                "@id": "caqouc:components/ActorQueryOperationCreate.jsonld#ActorQueryOperationCreate_args_mediatorQueryOperation"
              }
            },
            {
              "keyRaw": "name",
              "value": {
                "@id": "caqouc:components/ActorQueryOperationCreate.jsonld#ActorQueryOperationCreate_args_name"
              }
            },
            {
              "keyRaw": "bus",
              "value": {
                "@id": "caqouc:components/ActorQueryOperationCreate.jsonld#ActorQueryOperationCreate_args_bus"
              }
            },
            {
              "keyRaw": "busFailMessage",
              "value": {
                "@id": "caqouc:components/ActorQueryOperationCreate.jsonld#ActorQueryOperationCreate_args_busFailMessage"
              }
            },
            {
              "keyRaw": "beforeActors",
              "value": {
                "@id": "caqouc:components/ActorQueryOperationCreate.jsonld#ActorQueryOperationCreate_args_beforeActors"
              }
            }
          ]
        }
      ]
    },
    {
      "@id": "caqouc:components/ActorQueryOperationCreate.jsonld#IActorQueryOperationCreateArgs",
      "@type": "AbstractClass",
      "requireElement": "IActorQueryOperationCreateArgs",
      "extends": [
        "cbqo:components/ActorQueryOperationTypedMediated.jsonld#IActorQueryOperationTypedMediatedArgs"
      ],
      "parameters": [],
      "memberFields": [
        {
          "@id": "caqouc:components/ActorQueryOperationCreate.jsonld#IActorQueryOperationCreateArgs__member_mediatorUpdateQuads",
          "memberFieldName": "mediatorUpdateQuads"
        }
      ],
      "constructorArguments": []
    }
  ]
}