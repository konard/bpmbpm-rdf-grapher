{
  ...
  "components": [
    ...
    },
    {
      ...
      "comment": "Keys that can be used within an {@link IActionContext}.  To avoid entry conflicts, all keys must be properly namespaced using the following convention: Each key name must be prefixed with the package name followed by a `:`. For example, the `query-operation` bus declares the `operation` entry, which should be named as `@comunica/bus-query-operation:operation`.",
      ...
      "memberFields": [
        ...
        {
          "@id": "ct:components/IActionContext.jsonld#IActionContextKey__member_dummy",
