...
function materializeOperation(operation, bindings, algebraFactory, bindingsFactory, options = {}) {
    ...
    return utils_algebra_1.algebraUtils.mapOperation(operation, {
        [utils_algebra_1.Algebra.Types.PATH]: {
            preVisitor: () => ({ continue: false }),
            transform: pathOp => 
            // Materialize variables in a path expression.
            // The predicate expression will be recursed.
            Object.assign(algebraFactory.createPath(materializeTerm(pathOp.subject, bindings), pathOp.predicate, materializeTerm(pathOp.object, bindings), materializeTerm(pathOp.graph, bindings)), { metadata: pathOp.metadata }),
        },
        [utils_algebra_1.Algebra.Types.PATTERN]: {
            preVisitor: () => ({ continue: false }),
            transform: patternOp => 
            // Materialize variables in the quad pattern.
            Object.assign(algebraFactory.createPattern(materializeTerm(patternOp.subject, bindings), materializeTerm(patternOp.predicate, bindings), materializeTerm(patternOp.object, bindings), materializeTerm(patternOp.graph, bindings)), { metadata: patternOp.metadata }),
        },
        [utils_algebra_1.Algebra.Types.JOIN]: {
            ...
            Object.assign(algebraFactory.createJoin(op.input.map(input => materializeOperation(input, bindings, algebraFactory, bindingsFactory, options)), op.input.every(input => !input.metadata)), { metadata: op.metadata }),
        },
        [utils_algebra_1.Algebra.Types.EXTEND]: { transform: (extendOp) => {
                ...
                if (bindings.has(extendOp.variable)) {
                    if (options.strictTargetVariables) {
                        throw new Error(`Tried to bind variable ${(0, rdf_string_1.termToString)(extendOp.variable)} in a BIND operator.`);
                    }
                    else {
        ...
        [utils_algebra_1.Algebra.Types.GROUP]: { transform: (groupOp) => {
                ...
                return algebraFactory.createGroup(groupOp.input, variables, groupOp.aggregates);
            } },
        [utils_algebra_1.Algebra.Types.FILTER]: {
            preVisitor: () => ({ continue: false }),
            transform: (filterOp) => {
                ...
                const values = createValuesFromBindings(algebraFactory, originalBindings);
                // Recursively materialize the filter expression
                const recursionResultExpression = materializeOperation(filterOp.expression, bindings, algebraFactory, bindingsFactory, options);
                // Recursively materialize the filter input
                let recursionResultInput = materializeOperation(filterOp.input, bindings, algebraFactory, bindingsFactory, options);
        ...
        [utils_algebra_1.Algebra.Types.PROJECT]: {
            ...
            transform: (projectOp) => {
                ...
                const values = createValuesFromBindings(algebraFactory, options.originalBindings, projectOp.variables);
                let recursionResult = materializeOperation(projectOp.input, bindings, algebraFactory, bindingsFactory, options);
                if (values.length > 0) {
        ...
        [utils_algebra_1.Algebra.Types.VALUES]: {
            ...
            transform: (valuesOp) => {
                ...
                const valueBindings = valuesOp.bindings.map((binding) => {
            ...
            preVisitor: (expressionOp) => {
                ...
                if (expressionOp.subType === 'operator' &&
                    (expressionOp.operator === 'bound' && expressionOp.args.length === 1 &&
                        ...
                        .some(variable => expressionOp.args[0].term.equals(variable)))) {
                    return { continue: false };
                }
            ...
            transform: (expressionOp) => {
                ...
                if (expressionOp.subType === 'operator') {
                    if (expressionOp.operator === 'bound' && expressionOp.args.length === 1 &&
                        expressionOp.args[0].subType === 'term' && [...bindings.keys()]
                        .some(variable => expressionOp.args[0].term.equals(variable))) {
                        return algebraFactory.createTermExpression(algebraFactory.dataFactory.literal('true', algebraFactory.dataFactory.namedNode('http://www.w3.org/2001/XMLSchema#boolean')));
                    }
                    return expressionOp;
                }
                if (expressionOp.subType === 'aggregate' &&
                    ...
                    if (options.strictTargetVariables) {
                        throw new Error(`Tried to bind ${(0, rdf_string_1.termToString)(expressionOp.variable)} in a ${expressionOp.aggregator} aggregate.`);
                    }
                    else {
                        return expressionOp;
                    }
...
function createValuesFromBindings(factory, bindings, variables) {
    ...
    for (const [variable, binding] of bindings) {
        if (!variables || variables.some(v => v.equals(variable))) {
            const newBinding = { [variable.value]: binding };
