...
let techAppendixData = {
    ...
    contextTriGTypes: {},
    quads: []  // Квады из tech_Appendix.ttl для добавления в общий quadstore (Вариант 2)
};
...
/**
 * Парсит содержимое vad-basic-ontology_tech_Appendix.ttl
 * Отдельная функция для повторного использования при выборе файла вручную
 * @param {string} ttlContent - Содержимое TTL файла
 */
async function parseTechAppendix(ttlContent) {
    ...
    // Сохраняем квады для добавления в общий quadstore (Вариант 2)
    // Преобразуем квады, добавляя граф vad:techtree
    techAppendixData.quads = techQuads.map(quad => {
        return N3.DataFactory.quad(
            quad.subject,
            quad.predicate,
            quad.object,
            N3.DataFactory.namedNode(TECHTREE_GRAPH_URI)
        );
    });
